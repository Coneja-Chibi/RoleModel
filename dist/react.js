...
var require_latest = __commonJS({
  "snapshots/latest.json"(exports$1, module) {
    module.exports = {
      _meta: {
        fetchedAt: "2026-04-02T07:22:53.396Z",
        modelCount: 350,
        providerCount: 56,
        source: "openrouter"
      },
      data: [
        ...
        },
        {
          ...
          created: 1775058318,
          description: "Trinity Large Thinking is a powerful open source reasoning model from the team at Arcee AI. It shows strong performance in PinchBench, agentic workloads, and reasoning tasks. It is free in open claw for the first five days. Launch video: https://youtu.be/Gc82AXLa0Rg?si=4RLn6WBz33qT--B7",
        ...
        },
        {
          ...
          created: 1774979158,
          description: "Grok 4.20 Multi-Agent is a variant of xAI\u2019s Grok 4.20 designed for collaborative, agent-based workflows. Multiple agents operate in parallel to conduct deep research, coordinate tool use, and synthesize information across complex tasks.\n\nReasoning effort behavior:\n- low / medium: 4 agents\n- high / xhigh: 16 agents",
          context_length: 2e6,
          architecture: {
        ...
        },
        {
          ...
          created: 1774907255,
          description: "30 second duration clips are priced at $0.04 per clip. Lyria 3 is Google's family of music generation models, available through the Gemini API. With Lyria 3, you can generate high-quality, 48kHz stereo audio from text prompts or from images. These models deliver structural coherence, including vocals, timed lyrics, and full instrumental arrangements. Lyria 3 Clip can generate short clips, loops, previews.",
          context_length: 1048576,
          architecture: {
            modality: "text+image->text+audio",
            input_modalities: [
        ...
        },
        {
          ...
          pricing: {
            prompt: "0.0000003827",
            completion: "0.0000019092",
            input_cache_read: "0.0000001935"
          },
          top_provider: {
            context_length: 262144,
            max_completion_tokens: 262144,
        ...
        },
        {
          ...
          created: 1755095639,
          description: "Mistral Medium 3.1 is an updated version of Mistral Medium 3, which is a high-performance enterprise-grade language model designed to deliver frontier-level capabilities at significantly reduced operational cost. It balances state-of-the-art reasoning and multimodal performance with 8\xD7 lower cost compared to traditional large models, making it suitable for scalable deployments across professional and industrial use cases.\n\nThe model excels in domains such as coding, STEM reasoning, and enterprise adaptation. It supports hybrid, on-prem, and in-VPC deployments and is optimized for integration into custom workflows. Mistral Medium 3.1 offers competitive accuracy relative to larger models like Claude Sonnet 3.5/3.7, Llama 4 Maverick, and Command R+, while maintaining broad compatibility across cloud environments.",
          context_length: 131072,
          architecture: {
            modality: "text+image->text",
            input_modalities: [
        ...
        },
        {
          ...
          created: 1751986985,
          description: "DeepSeek-TNG-R1T2-Chimera is the second-generation Chimera model from TNG Tech. It is a 671 B-parameter mixture-of-experts text-generation model assembled from DeepSeek-AI\u2019s R1-0528, R1, and V3-0324 checkpoints with an Assembly-of-Experts merge. The tri-parent design yields strong reasoning performance while running roughly 20 % faster than the original R1 and more than 2\xD7 faster than R1-0528 under vLLM, giving a favorable cost-to-intelligence trade-off. The checkpoint supports contexts up to 60 k tokens in standard use (tested to ~130 k) and maintains consistent <think> token behaviour, making it suitable for long-context analysis, dialogue and other open-ended generation tasks.",
          context_length: 163840,
          architecture: {
            modality: "text->text",
            input_modalities: [
...
function getSizeTier(contextLength) {
  if (contextLength < 8e3) return "tiny";
  if (contextLength < 32e3) return "small";
  if (contextLength < 128e3) return "medium";
...
async function loadFromSnapshot() {
  try {
    const snapshot = await Promise.resolve().then(() => __toESM(require_latest()));
...
// src/utils.ts
function findModel(query, models) {
...
function getContextLimit(modelId, models) {
  const model = findModel(modelId, models);
  return model?.contextLength;
}
function queryModels(models, options) {
  let results = [...models];
  if (options.provider) {
  ...
  if (options.search) {
    ...
    results = results.filter(
      (m) => m.id.toLowerCase().includes(search) || m.name.toLowerCase().includes(search) || m.provider.name.toLowerCase().includes(search) || m.description?.toLowerCase().includes(search)
    );
  }
  if (options.sortBy) {
    ...
    results.sort((a, b) => {
      switch (options.sortBy) {
        case "context":
          return (a.contextLength - b.contextLength) * order;
        case "price":
          return (a.pricing.promptPerMillion - b.pricing.promptPerMillion) * order;
        case "name":
          return a.name.localeCompare(b.name) * order;
        case "provider":
          return a.provider.name.localeCompare(b.provider.name) * order;
        default:
          return 0;
      }
    });
...
// src/react.ts
var CACHE_KEY = "llm-model-registry";
function loadFromCache() {
  ...
  try {
    ...
    if (/* @__PURE__ */ new Date() >= expiresAt) return null;
    return registry;
  } catch {
    return null;
  }
}
function saveToCache(registry) {
...
function useModelRegistry() {
  ...
  const loadRegistry = react.useCallback(async (forceRefresh = false) => {
  ...
  const getContextLimitFn = react.useCallback(
    (id) => {
      if (!registry) return void 0;
      return getContextLimit(id, registry.models);
    },
    [registry]
  );
  const queryModelsFn = react.useCallback(
    (options) => {
      if (!registry) return [];
  ...
  const getProvidersFn = react.useCallback(() => {
    if (!registry) return [];
...
function useContextLimit(modelId) {
  const { getContextLimit: getContextLimit2, isLoading } = useModelRegistry();
  const contextLimit = react.useMemo(() => {
    if (!modelId) return void 0;
    return getContextLimit2(modelId);
  }, [modelId, getContextLimit2]);
  return { contextLimit, isLoading };
}
function useProviderModels(providerId) {
  const { queryModels: queryModels2, isLoading } = useModelRegistry();
  const models = react.useMemo(() => {
    if (!providerId) return [];
    return queryModels2({ provider: providerId, sortBy: "context", sortOrder: "desc" });
  }, [providerId, queryModels2]);
  return { models, isLoading };
}

exports.useContextLimit = useContextLimit;
exports.useModel = useModel;
exports.useModelRegistry = useModelRegistry;
exports.useProviderModels = useProviderModels;
